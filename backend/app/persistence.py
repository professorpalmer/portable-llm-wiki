...
def _ensure_tenant_gitignore(root: Path) -> None:
    """Make sure tenant runtime files are in ``<root>/.gitignore``.
    ...
    short version: these are load-bearing runtime exclusions, and they
    have to work even when the cloned repo already has unrelated gitignore
    content.
    """
    gi = root / ".gitignore"
    ...
    if not existing:
        addendum_lines.append("# Auto-generated by portable-llm-wiki.")
    if "tenant.json" in missing_rules:
    ...
    if ".share-token-stats.json" in missing_rules:
        addendum_lines.extend(
            [
...
def _discard_share_token_bookkeeping_dirt(cwd: Path) -> None:
    """Clear share-token bookkeeping dirt without losing sidecar counters.

    Legacy ``.share-tokens.json`` hit bumps are restored from HEAD.
    ``.share-token-stats.json`` counters are preserved: never ``checkout``
    the sidecar (that would discard local hits). Clear any staged index
    noise from older remediation and mark the path ``skip-worktree`` so
    porcelain stays clean for the fast-forward.
    """
    stats_path = cwd / ".share-token-stats.json"
    ...
    # Never checkout the sidecar worktree — only undo staged deletions.
    _run_git(["restore", "--staged", ".share-token-stats.json"], cwd=cwd)
    if stats_bytes is not None:
        try:
            ...
            pass
    _run_git(
        ["update-index", "--skip-worktree", ".share-token-stats.json"], cwd=cwd
    )
...
def classify_pull_safety(tenant: "Tenant") -> dict:
    ...
    if tracked and _is_share_tokens_hits_only_dirt(root, tracked):
        blocking_tracked = []
    out.update(
        ok=True,
        behind=behind,
        ...
        tracked_modified=blocking_tracked,
        untracked=untracked,
    )
    ...
    elif blocking_tracked:
        out["reason"] = (
            f"behind by {behind}, {len(blocking_tracked)} local edit(s) at risk"
        )
    else:
        out["auto_ff"] = True
        out["reason"] = (
